...
description: "Reflective pass over the memory files — merges duplicate entries, retires dated/expired memories, converts relative time references to absolute dates, prunes thin or easily re-discoverable notes, and tidies the MEMORY.md index so a future session can orient quickly without re-asking. Use when the user signals memory housekeeping — phrases like 'consolidate memory', 'clean up memory', 'tidy memory files', 'merge memory entries', 'memory is getting messy', 'prune memory index', 'reflect on what you know about me', or when a session-end review surfaces stale or duplicated memory. Do not invoke for live retrieval of memory in the current conversation — only for the housekeeping pass itself."
---
